...
0x00,0x00,0x00,0x80,0x80,0x80,0xFE,0x40,0x40,0x40,0x20,0x20,0x20,0x00,
0x00,0x01,0x01,0x00,0x00,0x00,0x3F,0x40,0x40,0x40,0x40,0x40,0x3C,0x00,//七
0x00,0x80,0x80,0xFE,0xA2,0xA2,0xA2,0xA2,0xA2,0xA2,0xBE,0x80,0x80,0x00,
0x00,0x00,0x00,0x00,0x06,0x05,0x04,0x44,0x44,0x64,0x1E,0x00,0x00,0x00,//号
...
//OLED底层函数
void LEDPIN_Init(void)    //初始化引脚口 ，均为普通引脚输出
{
    LED_SCL_Init;
    LED_SDA_Init;
    LED_RST_Init;
...
void LED_WrDat(uint8 data)   //写数据函数
{
    ...
    LED_SCLL;
    buff.Value=data;
    SDA_OUT=buff.wei.bit7;LED_SCLH;LED_SCLL;
    SDA_OUT=buff.wei.bit6;LED_SCLH;LED_SCLL;
    SDA_OUT=buff.wei.bit5;LED_SCLH;LED_SCLL;
    SDA_OUT=buff.wei.bit4;LED_SCLH;LED_SCLL;
    SDA_OUT=buff.wei.bit3;LED_SCLH;LED_SCLL;
    SDA_OUT=buff.wei.bit2;LED_SCLH;LED_SCLL;
    SDA_OUT=buff.wei.bit1;LED_SCLH;LED_SCLL;
    SDA_OUT=buff.wei.bit0;LED_SCLH;LED_SCLL;
}
...
void LED_Fill(uint8 bmp_data)
{
	...
	for(y=0;y<8;y++)
	{
		LED_WrCmd(0xb0+y);   //0xb0+0~7 表示0～7页
		LED_WrCmd(0x00);       //0x00+0~16表示将128列分为16组其他地址在某组中的第几列
		LED_WrCmd(0x10);	     //0x10+0~16表示将128列分为16组其地址所在第几组
		for(x=0;x<128;x++)
			LED_WrDat(bmp_data);
	}
}
...
//写入一组6x8标准ASCII字符串
//x为显示的横坐标0～122，y为页范围0～7， 要显示的字符串
...
//写入一组8x16标准ASCII字符串
//x为显示的横坐标0～120，y为页范围0～3， 要显示的字符串
...
//写入一组14x16的中文字符串
//x为显示的横坐标0～114，y为页范围0～3， 要显示的中文字符串
...
//显示BMP图片 大小为(x1-x0+1) X ((y1-y0+1)*8)
//BMP图片先取页码y为0 以8位（低位在上）为单位取满（x1-x0+1）列
//再取页码y为1以8位（低位在上）为单位取满（x1-x0+1）列以此类推
//x的范围为0～127 ，y为页的范围0～7
...
//将一个char型数转换成3位数进行显示
// x:0~122  y为页的范围0～7 data为需要转化显示的数值
...
void LED_PrintValueC2(u8 x, u8 y, char data)
{
	u8 i,j;
	i = data/10;
	j = data%10;
	
	LED_P6x8Char(x,y,i+48);
...
//4位显示
void OLED_PrintValueI4(u8 x, u8 y,signed int data)
{
	u8 i,j,l;
	if(data < 0)
	{
...
void LED_PrintImage(uint8 *pucTable, uint16 usRowNum, uint16 usColumnNum)
{
    ...
    LED_Set_Pos(0,(uint8)i); //设置剩下的行显示的其实坐标
    usRowTmp = i << 3;       //计算当前所在行的下标
    for(j = 0; j < usColumnNum; j++) //列扫描
    {
        ucData = 0;
...
void showI(int x,int y,unsigned char s[],int c)
{
    ...
    //for(i=4;i<5;i++)
        //table[i]=' ';
    LED_P6x8Str((byte)x,(byte)y,(byte*)s);
    LED_P6x8Str((byte)x+72,(byte)y,(byte*)table);  
...
{
    LED_P6x8Str((byte)x,(byte)y,(byte*)s);
    LED_PrintValueF((byte)x+66,(byte)y,data,(u8)a);
}
...
{
    int i;
    //int scope_limit = D_VALUE(scope_top,scope_buttom);
    int scope_limit=0;  //防止D_VALUE报错才设置的
    static int data_arr[100] = {0};
    int count = 0;
    uint8 ucData;
    int x,y;
    ...
    //坐标
    for(i = 0;i < 8;i++)
        LED_PrintValueI(0,i,scope_top - scope_limit*i/7);
    
    for(i = 0; i < 8; i++)
    {
        LED_Set_Pos(37,(uint8)i);
        LED_WrDat(0x00);
        LED_Set_Pos(38,(uint8)i);
        LED_WrDat(0xff);
    }
  
    for(i = 0; i < 8; i++)
    {
        LED_Set_Pos(39,(uint8)i);
        for(x = 39; x < 128 ; x++)
        {
            ucData = 0;
            ...
            for(y = i*8;y < (i+1)*8;y++)
            {
                ...
                ucData = ucData>>1;
                if(data_arr[128 - x] >= scope_top - (y+1)*scope_limit/64 && data_arr[128 - x] < scope_top - (y-1)*scope_limit/64)
                {
                    ucData |=0x80;
                }
                
                if(scope_center >= scope_top - (y+1)*scope_limit/64 && scope_center < scope_top - (y-1)*scope_limit/64)
                {
                    count ++;
                    if(count > 10) count = 0;
                    if(count > 5)
